...
function runQuery()
{
    ...
    try {
        ...
        schema = uiTemplateData.modules[sitepos].schema;
...
function populateShowHide(resultsJSON){
	...
	for (var i = 1; i < columns.length; i++)
    {
        ...
        option.innerHTML = columns[i].label;
		option.title = columns[i].label;
...
function populateSources()
{
	...
	var source = createSource(uiTemplateData.name);
...
function populateSites()
{
    ...
    var modulesArray = uiTemplateData.modules;
    var i;
...
function populateSiteAttributeOptionsAux(siteIndex)
{
    ...
    var attributeSetArray = uiTemplateData.attributes;
    var attributes;
    ...
    for (i = 0; i < attributes.length; i++)
    {
        ...
        option.innerHTML = attributes[i].desc;
		option.title = attributes[i].desc;
...
{
	var cfg = getConfig();
	var startd = cfg.datehandler.createDate(2017,0,1);
    var startdLong = startd.getTime();
    var nextLong = startdLong + (24 * 60 * 60 * 1000);
    var stopd = cfg.datehandler.createDate(nextLong);
	setStartDate(startd);
	setStopDate(stopd);  
}
