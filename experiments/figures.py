...
def _irr_correlations(start, stop, output):
    ...
    targets_query = f'SELECT TIMESTAMP as reftime, ' \
                    f'AVG(UGAAPOA1IRR) as Array_A, AVG(UGABPOA1IRR) as Array_B, AVG(UGAEPOA1IRR) as Array_E ' \
                    f'FROM IRRADIANCE WHERE reftime BETWEEN {unix_start} AND {unix_stop} ' \
                    f'AND hour=18 ' \
                    f'GROUP BY hour, day, month, year;'
    # load data and aggregate by hour
    targets_ds = pd.read_sql_query(sql=targets_query, con=connection, index_col='reftime', parse_dates=['reftime'])
    targets_ds = targets_ds.dropna()
    targets_ds.index = pd.to_datetime(targets_ds.index, unit='s')  # timestamp index is unix epoch (in seconds)
    ...
    cloud_cover_ds = SolarDataset(start=start, stop=stop, target=None, forecast=0,
                                  feature_subset=(cloud_cover_var, air_temp_var, downward_flux_var),
                                  temporal_features=False, geo_shape=(1,1), standardize=False, target_hours=0).xrds

    # we have only selected data along the reftime coordinate, so the others can be dropped
    cloud_cover_ds = cloud_cover_ds.drop(labels=('x', 'y', 'lat', 'lon', 'forecast', 'z_EATM')).squeeze()

    dataset = xr.merge([targets_ds, cloud_cover_ds], join='inner')

    array_a, array_b, array_e = dataset['Array_A'].values, dataset['Array_B'].values, dataset['Array_E'].values
...
def run(output='./output/figures', start='2017-01-01', stop='2018-12-31'):
    outpath = pathlib.Path(output).resolve()
    outpath.mkdir(parents=True, exist_ok=True)

    _irr_vs_hour(start=start, stop=stop, output=outpath)
    _irr_vs_month(start=start, stop=stop, output=outpath)
    _irr_correlations(start=start, stop=stop, output=outpath)
