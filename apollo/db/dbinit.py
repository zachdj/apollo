"""
Python convenience script for generating a SQLite database for the UGA solar farm. 

A set of hard-coded SQL statements is used.
"""
import apollo.db.dbapi as db
import argparse
import logging
...
def init_solar_db(db_file):
    """Initialize the solar farm database schema, using the given database file name and creation script. 
    """
    logging.info("db init...")
    dbh = None
    db_file = str(db_file)
    try:
        dbh = db.DBHandler(db_file)
        logging.info("handler created...")
        dbh.connect()
        logging.info("connected...")
        logging.info("invoking script...")
        dbh.executescript(sql)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="""Utility function creating a SQLite datbase for the solar farm from a hard-coded SQL script.
                                     
    Example: >python -m apollo.db.dbinit solar_farm.db
    """)
    parser.add_argument('db', metavar='DB', type=str, nargs=1,help='the database file to create.')
